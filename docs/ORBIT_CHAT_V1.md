...
Assistant replies render headings, emphasis, lists, quotes and code as escaped React text nodes. Model-supplied HTML and Markdown links do not become active elements; source and asset links continue to come from server-validated cards. The project-status tool reports the active policy's AI provider spending limits in USD millionths, including whether the paid mandate is active; each reservation still checks remaining capacity. These limits are distinct from campaign or media spending, which the chat must leave unknown unless separately provided.
