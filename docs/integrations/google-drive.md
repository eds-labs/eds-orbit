...
4. Set `GOOGLE_DRIVE_CLIENT_ID` and `GOOGLE_DRIVE_CLIENT_SECRET` in the deployment secret store; Docker Compose passes them to the Orbit API and worker. Until both reach the API, Settings shows a disabled **Connect Google Drive** button and an explanation. Keep `CREDENTIAL_KEY` stable and secret; changing it without re-encrypting stored credentials invalidates connections. No real value belongs in `.env.example` or version control.
